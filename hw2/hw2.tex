...
%\begin{align*}
       ...
       %&\phi_c^{\dagger}(\mathbf{y})(\sigma^{k})_{cd}[\phi_a^{\dagger}(\mathbf{x}) ,\, \pi^{\dagger}_d(\mathbf{y})] (\sigma^{j})_{ab}\pi^{\dagger}_b(\mathbf{x})
        %+ \phi_a^{\dagger}(\mathbf{x})(\sigma^{j})_{ab}[\pi^{\dagger}_b(\mathbf{x}) ,\, \phi_c^{\dagger}(\mathbf{y})] (\sigma^{k})_{cd}\pi^{\dagger}_d(\mathbf{y})\\
       %&+\pi_c(\mathbf{y}) (\sigma^{k})_{cd}[ \pi_a(\mathbf{x}),\, \phi_d(\mathbf{y})] (\sigma^{j})_{ab}\phi_b(\mathbf{x})
       %+ \pi_a(\mathbf{x}) (\sigma^{j})_{ab}[ \phi_b(\mathbf{x}),\, \pi_c(\mathbf{y}) ](\sigma^{k})_{cd}\phi_d(\mathbf{y})
        %\end{aligned} \\
        %&= -\frac{i}{4}\int_{\mathbf{x}} 
        %\begin{aligned}[t]
       %&\phi_c^{\dagger}(\mathbf{y})(\sigma^{k})_{cd}\delta_{ad}(\sigma^{j})_{ab}\pi^{\dagger}_b(\mathbf{x})
        %- \phi_a^{\dagger}(\mathbf{x})(\sigma^{j})_{ab}\delta_{bc} (\sigma^{k})_{cd}\pi^{\dagger}_d(\mathbf{y})\\
       %&+\pi_c(\mathbf{y}) (\sigma^{k})_{cd}\delta_{ad}(\sigma^{j})_{ab}\phi_b(\mathbf{x})
       %- \pi_a(\mathbf{x}) (\sigma^{j})_{ab}\delta_{bc}(\sigma^{k})_{cd}\phi_d(\mathbf{y})
        %\end{aligned} \\
        %&= -\frac{i}{4}\int_{\mathbf{x}} 
        %\begin{aligned}[t]
       %&\phi_a^{\dagger}(\mathbf{y})(\sigma^{k})_{ab}(\sigma^{j})_{bc}\pi^{\dagger}_c(\mathbf{x})
        %- \phi_a^{\dagger}(\mathbf{x})(\sigma^{j})_{ab}(\sigma^{k})_{bc}\pi^{\dagger}_c(\mathbf{y})\\
       %&+\pi_a(\mathbf{y}) (\sigma^{k})_{bc}(\sigma^{j})_{cd}\phi_d(\mathbf{x})
       %- \pi_a(\mathbf{x}) (\sigma^{j})_{ab}(\sigma^{k})_{bc}\phi_c(\mathbf{y})
        %\end{aligned} \\
        %&= \frac{i}{4} \int_{\mathbf{x}} 
        %\phi_a^{\dagger}(\mathbf{x})[\sigma^{j},\, \sigma^{k}]_{ab}\pi^{\dagger}_b(\mathbf{y})
       %+\pi_a(\mathbf{y}) (\sigma^{k})_{bc}(\sigma^{j})_{cd}\phi_d(\mathbf{x})
       %- \pi_a(\mathbf{x}) (\sigma^{j})_{ab}(\sigma^{k})_{bc}\phi_c(\mathbf{y})
%\end{align*}
%\endgroup
%où on a appliquer le delta de Kronecker et renommer les indices pour simplifier la notation. 
%Pour  progresser, on rappel la relation de commutation des matrices de Pauli, soit
%\begin{equation}
        %[\sigma^{j}, \sigma^{k}] = 2i \epsilon\indices{^{jk}_{\ell}} \sigma^{\ell}
%\end{equation} 
%La constante de structure de l'algèbre de Lie associé à $SU(2)$ est donc $f\indices{^{jk}_{\ell}} = 2 i\epsilon\indices{^{jk}_{\ell}}$, où $\epsilon\indices{^{jk}_{\ell}}$ est tenseur de Levi-Civita. 
%\begin{align*}
        %[Q^{j}, Q^{k}] 
        %&= -\frac{1}{2} \epsilon\indices{^{jk}_{\ell}}\int_{\mathbf{x}} 
        %\phi_a^{\dagger}(\mathbf{x})[\sigma^{j}\sigma^{k}]_{ab}\pi^{\dagger}_b(\mathbf{y})
       %+\pi_a(\mathbf{y}) (\sigma^{k})_{bc}(\sigma^{j})_{cd}\phi_d(\mathbf{x})
       %- \pi_a(\mathbf{x}) (\sigma^{j})_{ab}(\sigma^{k})_{bc}\phi_c(\mathbf{y})
%\end{align*}
